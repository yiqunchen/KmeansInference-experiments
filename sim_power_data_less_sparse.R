...
for (delta in delta_seq){
  for (p in p_seq){
    for (sig in sig_seq){
      ...
      theta_matrix <- (as.matrix(pracma::randortho(floor(p*0.1),
                       type = c("orthonormal"))[,c(1:k)]/sqrt(2)*(delta)))

      theta_matrix_aug <- rbind(theta_matrix, matrix(0,ncol=k,nrow=(p-nrow(theta_matrix))))

      mu <- t(theta_matrix_aug)
      ...
      save(result_df, naive_vec,nu_norm,selective_vec,selective_vec_sample,selective_vec_MED,true_mu_diff,adjust_rand_index,
           file=paste0(output_dir,"less_sparse_validation_sim","_k_",k,"_delta_",delta,"_sigma_", sig, "_p_",p,".RData"))
    }
  }
}
